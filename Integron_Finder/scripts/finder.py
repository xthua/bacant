...
import os
import sys
new_python_path = sys.path[0]+"/Integron_Finder"
...
def run_integron_finder(current_dir,resultdir,nucleotide,args=None, loglevel=None):
    ...
    args = ("--cmsearch %s --hmmsearch %s --prodigal %s --outdir %s --cpu 4 --func-annot --local-max -qq "%(cmsearch,hmmsearch,prodigal,resultdir+"/integron")+nucleotide).split(" ")
