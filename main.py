...
from Integron_Finder.scripts.finder import run_integron_finder
...
def findResistanceGene(inFile,outdir,blastnPath,database,default_ident,default_cov):
    ...
    cmd = blastnPath + ' -task blastn -dust no -evalue 1E-20 -culling_limit 1 -query ' + inFile + ' -db ' + database + ' -outfmt \"6 sseqid pident length  qstart qend slen sstrand qseqid gaps\" -out ' + outdir + '/blast.out'
    ...
    if len(lines) == 0:
        ...
        for line in lines:
            ...
            data = str(ident)+'|'+str(cov)+'|'+leftPos+'|'+rightPos+'|'+name+'|'+strand+'|'+data[-2]+'|'+data[-1]+'|'+product+'|'+acc+'|'+covlen
...
def integron_finder(current_dir,nucleotide,resultdir,blastn_path,database):
    cmsearch = current_dir+"/Integron_finder/infernal/cmsearch"
    hmmsearch = current_dir+"/Integron_finder/hmmsearch/hmmsearch"
    prodigal = current_dir+"/Integron_finder/prodigal/prodigal"
    try:
        merged_integron_path = run_integron_finder(current_dir,resultdir,nucleotide)
        #print(merged_integron_path)
...
def parse_integron_result(resultdir,merged_integron_path):
    ...
    if merged_integron_path:
        ...
        #shutil.rmtree(resultdir+"/integron")
        ...
        if lines:
            ...
            if flag == "AMR":
                ...
                if int(line[8]) < int(line[9]):
                    ...
                    strand = "-"
            #feature
            features = ["gene","CDS","mobile_element","tRNA"]
            if flag == "AMR":
                if db_xref == "":
                    with open(outfile,"a") as w:
            ...
            if flag == "transposon":
                if db_xref == "":
                    with open(outfile,"a") as w:
                        w.write(filename+'\t'+source+'\t'+features[3]+'\t'+start+'\t'+end+'\t.\t'+strand+'\t.\tlocus_tag='+locus+';tRNA='+CDS+';gene='+gene+'\n')
                else:
                    with open(outfile,"a") as w:
                        w.write(filename+'\t'+source+'\t'+features[3]+'\t'+start+'\t'+end+'\t.\t'+strand+'\t.\tlocus_tag='+locus+';tRNA='+CDS+';db_xref='+db_xref+';gene='+gene+'\n')
...
def run():
    ...
    if blastn_path and blastp_path:
        ...
        if indir:
            for file in os.listdir(indir):
                ...
                if IntegronDB:
                    ...
                    try:
                        ...
                        with open(outdir+"/integron.finished",'w') as w:
                            w.write("abnormal exit")
                        print("integron_finder error")
                ...
                if os.path.exists(outdir+"/integron"):
                    os.system("rm -rf %s/integron"%outdir)
                #-----
        else:
            #genbank input
            if genbank:
            ...
            if os.path.exists(resultdir+"/integron"):
                os.system("rm -rf %s/integron"%resultdir)
            #-----
